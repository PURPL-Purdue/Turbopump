...
def rotor_back_calculate(RPM, torque, mass_flow, beta, radius, V_in):
    ...
    eq5 = sp.Eq(V_in * sp.sin(a1) - v2 * sp.sin(a2), C)
...
def add_angle_annotation(ax, origin, angle1, angle2, radius, label):
    """Draw a circular arc showing the angle between angle1 and angle2."""
    a1 = np.degrees(angle1)
    a2 = np.degrees(angle2)
    ...
    label_x = origin[0] + 1.2 * radius * np.cos(mid_angle)
    label_y = origin[1] + 1.2 * radius * np.sin(mid_angle)
...
def plot_velocity_triangles(
    ...
    # === Restored your original labels ===
    ax.quiver(
        0, 0, w1_vec[0], w1_vec[1],
        angles="xy", scale_units="xy", scale=1,
        color="r",
        label=f"w1 = {w1:.3f} m/s"
    )
    ax.quiver(
        w1_vec[0], w1_vec[1], u1_vec[0], u1_vec[1],
        angles="xy", scale_units="xy", scale=1,
        color="b",
        label=f"u1 = {u:.3f} m/s"
    )
    ax.quiver(
        0, 0, v1_vec[0], v1_vec[1],
        angles="xy", scale_units="xy", scale=1,
        color="g",
        label=f"v1 = {v1:.3f} m/s"
    )

    chord_vec = chord_length * np.array([np.cos(inclination_angle), np.sin(inclination_angle)])
    chord_end = w1_vec + chord_vec

    ax.quiver(
        chord_end[0], chord_end[1], w2_vec[0], w2_vec[1],
        angles="xy", scale_units="xy", scale=1,
        color="r",
        label=f"w2 = {w2:.3f} m/s"
    )
    ax.quiver(
        chord_end[0] + w2_vec[0], chord_end[1] + w2_vec[1], u2_vec[0], u2_vec[1],
        angles="xy", scale_units="xy", scale=1,
        color="b",
        label=f"u2 = {u:.3f} m/s"   # same magnitude as u1
    )
    ax.quiver(
        chord_end[0], chord_end[1], v2_vec[0], v2_vec[1],
        angles="xy", scale_units="xy", scale=1,
        color="g",
    ...
    add_angle_annotation(ax, origin=tuple(chord_end),
                         ...
                         radius=arc_radius, label=r"$\alpha_2$")
    add_angle_annotation(ax, origin=tuple(chord_end),
                         angle1=inclination_angle,
    ...
    angle_info = (
        ...
        r"$\alpha_2$ = " + f"{np.degrees(alpha2):.1f}°\n"
...
def plot_turbine(x_lower, y_lower, x_upper, y_upper, num_blades, hub_radius, chord, blade_radius, offset):
    rescale_factor = np.max([
        chord / abs(np.max(x_lower) - np.min(x_lower)),
        chord / abs(np.max(x_upper) - np.min(x_upper))
    ])
    
    x_lower = x_lower * rescale_factor
    y_lower = y_lower * rescale_factor
    x_upper = x_upper * rescale_factor
    y_upper = y_upper * rescale_factor
    ...
    fig.add_surface(x=X_cyl, y=Y_cyl, z=Z,
    ...
    fig.add_trace(go.Mesh3d(
        x=X_cap, y=Y_cap, z=np.full_like(X_cap, chord),
        color="blue", opacity=0.5, alphahull=0, showscale=False
    ))
    ...
    for k in range(num_blades):
        ...
        # Rotate in Y-Z plane
        X_rot = Y * np.cos(theta_blade) - Z * np.sin(theta_blade)
        Y_rot = Y * np.sin(theta_blade) + Z * np.cos(theta_blade)

        # Plot blades
        fig.add_surface(x=X_rot, y=Y_rot, z=X,
    ...
    fig.update_layout(
        ...
        title="Turbine with Hub and Blades"
    )
