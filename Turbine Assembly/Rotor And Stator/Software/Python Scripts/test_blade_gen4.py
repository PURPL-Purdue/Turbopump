...
def solve_for_x(b, r, y_2, x_2, initial_guess=0):
    """
    solves the equation (x+b)(r^2-(x+b)^2) = (y_2 - sqrt(r^2-(x+b)^2)) / (x_2 - x) for x.
    """
    def equation(x):
        left = (x + b) * (r**2 - (x + b)**2)
        right = (y_2 - np.sqrt(r**2 - (x + b)**2)) / (x_2 - x)
        return left - right
...
def compute_distances(x_lower, y_lower, upper_curve, target_distance, blade_spacing):
    ...
    sampled_indices = np.linspace(0, num_points//2, min(20, num_points), dtype=int)
    sse = 0
...
def update_plot(val):
    ...
    for line in ax.lines[3:]:
        line.remove()
    compute_distances_and_plot(x_lower[:num_points // 2], y_lower[:num_points // 2], full_upper[:num_points // 2, :], target_distance_percent * blade_spacing, blade_spacing, ax)
...
#     ax.plot(full_upper[:, 0], full_upper[:, 1], 'r-', label='Upper Surface')
#     ax.plot(x_lower, y_lower + blade_spacing, 'b-', label='Lower Surface Guide')
#     compute_distances_and_plot(x_lower[:num_points // 2], y_lower[:num_points // 2], full_upper[:num_points // 2, :], target_distance_percent * blade_spacing, blade_spacing, ax)
#     ax.legend()
#     fig.canvas.draw_idle()
###
...
def plot_geometry_with_sliders():
    ...
    compute_distances_and_plot(x_lower[:num_points // 2], y_lower[:num_points // 2], full_upper[:num_points // 2, :], initial_target_distance * initial_blade_spacing, initial_blade_spacing, ax)
