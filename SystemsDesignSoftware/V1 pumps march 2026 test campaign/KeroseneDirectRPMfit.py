...
#Pull pump curves
vol_flow, head = np.genfromtxt('Systems design software\\V1 pumps march 2026 test campaign\\CFTurbo performance curves\\Kero pump jetA\\Kero_jetA_20000_60000_combo.csv', delimiter='	', unpack=True, skip_header=1, dtype=float)
...
rpm_20000 = np.ones(99, int) * 20000
rpm_25000 = np.ones(100, int) * 25000
...
hstack_1 = np.hstack((rpm_20000, rpm_25000))
hstack_2 = np.hstack((hstack_1, rpm_30000))
hstack_3 = np.hstack((hstack_2, rpm_35000))
hstack_4 = np.hstack((hstack_3, rpm_40000))
hstack_5 = np.hstack((hstack_4, rpm_45000))
hstack_6 = np.hstack((hstack_5, rpm_50000))
hstack_7 = np.hstack((hstack_6, rpm_55000))
hstack_8 = np.hstack((hstack_7, rpm_60000))
...
ax.set_title('3D Regression Plot of kerosene pump curves')
...
prediction = func((66.49, 1422.51), a_fit, b_fit, c_fit, d_fit, e_fit, f_fit)
print(f'We need {prediction:.2f} RPM to meet the Kero setpoint')