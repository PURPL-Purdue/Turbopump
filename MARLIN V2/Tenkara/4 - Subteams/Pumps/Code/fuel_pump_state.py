...
def fuel_pump_state(mdot, safety_factor, shaft_speed_rpm, headrise, surrogate_flag, inlet_temperature, inlet_pressure, break_hp):
    g = 9.81
    ...
    if surrogate_flag == True:
        ...
        density = PropsSI('D', 'T', fluid_temp_kelvin, 'P', inlet_pressure, "Ethanol") # kg/m3
        vapor_pressure = PropsSI('P', 'T', fluid_temp_kelvin, 'Q', 0, 'Ethanol') #Pa


    vol_flow = mdot / density # m3 / s
    NPSHa = (inlet_pressure - vapor_pressure) / (density * g) # m
    target_NPSHr = NPSHa / safety_factor # m

    def inducer(vol_flow, target_NPSHr, shaft_speed_rpm, headrise):
        ...
        tip_flow_angle = np.rad2deg(np.arctan(meridional_velocity / tip_speed))
        hub_flow_angle = np.rad2deg(np.arctan(meridional_velocity / hub_tangential_speed))
    ...
    def impeller(shaft_speed_rpm, headrise, break_hp, vol_flow, target_NPSHr, density):
