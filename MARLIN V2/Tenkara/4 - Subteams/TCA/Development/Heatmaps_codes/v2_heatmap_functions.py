...
def get_isp_heatmap(ureg, of_range, Pc_range, reac_names, oxidant_weights, fuel_weights, reac_temps, Prat, ac_at):
    ...
    for i, of in enumerate(of_set):
        for j, Pc in enumerate(Pc_set):            
            solution = cea_setup(
                ...
                Prat = float((Pc / Pamb).to_base_units().magnitude),
                ac_at=ac_at)
...
def get_Tc_heatmap(ureg, of_range, Pc_range, reac_names, oxidant_weights, fuel_weights, reac_temps, Prat, ac_at):
    Pamb = 1 * ureg.bar
...
def get_isp_density_heatmap(ureg, rho_ox, rho_f, of_range, Pc_range, reac_names, oxidant_weights, fuel_weights, reac_temps, Prat, ac_at):
    g0 = 9.81 * ureg.m / (ureg.s)**2
    Pamb = 1 * ureg.bar
