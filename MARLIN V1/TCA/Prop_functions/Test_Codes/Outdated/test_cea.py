...
#Defining the operating temperatures and Pressures. At which we want to run the eq. problem.
pressures = np.array([1.0, 0.1, 0.01])*ureg.bar
temperatures = np.array([3000.0, 2000.0])*ureg.kelvin


print(pressures)
print(temperatures)

#Defining the input mole fractions for the fuel and ox. Also the array of equivalence ratios.
...
#Convert equivalence ratios to OF ratios.
...
#Going through the loop to solve for eq. at given T,P and OF.
for of_ratio in of_ratios:
    for p in pressures:
        for t in temperatures:
            weights = reac.of_ratio_to_weights(oxidant_weights, fuel_weights, of_ratio)
            solver.solve(solution, cea.TP, t.to(ureg.kelvin).magnitude, p.to(ureg.bar).magnitude, weights)

            # Store the output
            of_ratio_out[i] = of_ratio
            T_out[i] = t.to(ureg.kelvin).magnitude
            P_out[i] = p.to(ureg.atm).magnitude
